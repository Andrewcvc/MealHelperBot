...
async def go_back(callback: types.CallbackQuery, state: FSMContext):
    ...
    for step in AddDish.__all_states__:
        if step.state == current_state:
            ...
            await callback.message.edit_text(f'Ви повернулись на крок назад. \n{AddDish.text[previous_state]}')
            return
        previous_state = step
...
async def category_choice(callback:types.CallbackQuery, state:FSMContext):
    ...
    async with session_maker() as session:
        ...
        await callback.message.edit_text('<strong>Страву додано</strong>', reply_markup=get_user_added_btns(sizes=(2,)))
        await callback.answer()
        await state.clear()
...
async def add_dish_day(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with session_maker() as session:
        ...
        if not random_dish:
            categories = await orm_get_categories(session)
            kbds = get_callback_btns(btns={**{category.name: f'DayCategory_{category.id}' for category in categories}, "Головне меню🏠": UserAction(action=Action.main).pack()})
            await callback.message.edit_text("В цій категорії поки що немає страв. Виберіть іншу:", reply_markup=kbds)
        else:
            categories = await orm_get_categories(session)
            for catagory in categories:
                if catagory.id == category_id:
                    category_name = catagory.name
            await callback.message.edit_text(f'<strong>{category_name}:\n"{random_dish.name}"</strong>', reply_markup=get_callback_btns(
                btns={
...
async def set_algorithm(callback: types.CallbackQuery, state: FSMContext):
    async with session_maker() as session:
        ...
        await callback.message.edit_text('<strong>Оберіть категорію:</strong>', reply_markup=kbds)
...
async def add_weekly_category(callback: types.CallbackQuery, state: FSMContext):
    ...
    await state.update_data(category_id=category_id, message_id_to_delete=callback.message.message_id)
    await callback.message.edit_text('<strong>Введіть кількість страв для цієї категорії:</strong>', reply_markup=get_callback_btns(btns={'Відмінити❌': UserAction(action=Action.menu_for_week).pack()}))
...
async def clear_algorithm(callback: types.CallbackQuery):
    ...
    async with session_maker() as session:
        ...
        if result:
            await callback.message.edit_text("Ваші налаштування були успішно скинуті.", reply_markup=get_callback_btns(btns={"Задати новий алгоритм⚙️": UserAction(action=Action.set_algorithm).pack()}))
        else:
            await callback.message.edit_text("Не вдалося скинути налаштування. Спробуйте пізніше.", reply_markup=get_callback_btns(btns={"Задати новий алгоритм⚙️": UserAction(action=Action.set_algorithm).pack()}))
    await callback.answer()
